...
public class VaultManager {
    ...
    public void openStorageVault(Player player,int vault_type, int vault_num, boolean isCommand){
        ...
        CompletableFuture.runAsync(() -> {
            ...
            if(content != null){
                ...
                content.setHolderSettings(isCommand,only_pickup);
                vault = content.getInventory();
            } else {
                if(!perms){
                    sendMessages(player,No_permission_vault);
                    return;
                ...
                if(vault_type == 1){
                    container = new InventoryContainer(1);
                    VaultHolder_Large vaultHolderLarge = new VaultHolder_Large(vault_num);
                    vault = Bukkit.createInventory(vaultHolderLarge,54,color(vaultAttributesManager.replaceVaultTitle(player,1,vault_num)));
                    container.setInventory(vault);
                    vaultHolderLarge.setContainer(container);
                    vaultHolderLarge.setInventory(vault);
                    vaultHolderLarge.setIsCommand(isCommand);
                    vaultHolderLarge.setOnlyPickup(only_pickup);
                } else {
                    container = new InventoryContainer(2);
                    VaultHolder_Medium vaultHolderMedium = new VaultHolder_Medium(vault_num);
                    vault = Bukkit.createInventory(vaultHolderMedium,27,color(vaultAttributesManager.replaceVaultTitle(player,2,vault_num)));
                    container.setInventory(vault);
                    vaultHolderMedium.setContainer(container);
                    vaultHolderMedium.setInventory(vault);
                    vaultHolderMedium.setIsCommand(isCommand);
                    vaultHolderMedium.setOnlyPickup(only_pickup);
                }
                cacheInventoryManager.insertInventoryContainer(player,vault_type,vault_num,container);
            }
    ...
    public void openVaultIgnorePerms(Player player, int vault_type, int vault_num, CommandSender sender){
        ...
        CompletableFuture.runAsync(() -> {
            ...
            if(content != null){
                ...
                content.setHolderSettings(true,false);
                vault = content.getInventory();
            } else {
                jdbcUtil.insertVaultAsync(vault_type,player.getUniqueId().toString(),vault_num);
                if(vault_type == 1){
                    VaultHolder_Large vaultHolderLarge = new VaultHolder_Large(vault_num);
                    vaultHolderLarge.setContainer(new InventoryContainer(1));
                    vault = Bukkit.createInventory(vaultHolderLarge,54,color(vaultAttributesManager.replaceVaultTitle(player,1,vault_num)));
                    vaultHolderLarge.setInventory(vault);
                    vaultHolderLarge.setIsCommand(true);
                    vaultHolderLarge.setOnlyPickup(false);
                } else {
                    VaultHolder_Medium vaultHolderMedium = new VaultHolder_Medium(vault_num);
                    vaultHolderMedium.setContainer(new InventoryContainer(2));
                    vault = Bukkit.createInventory(vaultHolderMedium,27,color(vaultAttributesManager.replaceVaultTitle(player,2,vault_num)));
                    vaultHolderMedium.setInventory(vault);
                    vaultHolderMedium.setIsCommand(true);
                    vaultHolderMedium.setOnlyPickup(false);
                }
            }
